...
public class MainActivity extends MapActivity {
    ...
    private void initFiles(String area) {
        ...
        if (compressed.exists() && !compressed.isDirectory()) {
            try {
                ...
                Helper.unzip(compressed.getAbsolutePath(), GRAPH_FOLDER + "-gh",
                        deleteZipped);
    ...
    private Polyline createPolyline(GHResponse response) {
        ...
        for (GHPoint ghp : response.points()) {
            geoPoints.add(new GeoPoint(ghp.lat, ghp.lon));
        }
        ...
        new AsyncTask<Void, Void, GHResponse>() {
            ...
            protected void onPostExecute(GHResponse res) {
                log("from:" + fromLat + "," + fromLon + " to:" + toLat + "," + toLon
                        + " found path with distance:" + res.distance() / 1000f + ", nodes:"
                        + res.points().size() + ", time:" + time + " " + res.debugInfo());
                logUser("the route is " + (int) (res.distance() / 100) / 10f + "km long");
